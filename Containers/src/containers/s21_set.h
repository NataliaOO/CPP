...
template <typename value_type>
void Set<value_type>::erase(iterator it) {
  erase(it, root_);
}
...
template <typename value_type>
typename Set<value_type>::const_iterator Set<value_type>::begin() const {
  Node *node = root_;
  while (node && node->left) {
    node = node->left;
  }
...
typename Set<value_type>::iterator Set<value_type>::end() {
  return iterator(static_cast<typename Set<value_type>::Node *>(nullptr));
}

template <typename value_type>
typename Set<value_type>::const_iterator Set<value_type>::end() const {
  return const_iterator(static_cast<typename Set<value_type>::Node *>(nullptr));
}

template <typename value_type>
typename Set<value_type>::Node *Set<value_type>::copy(const Node *node,
                                                      Node *parent) {
  if (!node) {
    return nullptr;
  }
...
template <typename value_type>
void Set<value_type>::destroy_tree(Node *node) noexcept {
  if (node != nullptr) {
