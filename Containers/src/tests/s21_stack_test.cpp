...
TEST(Suite_Stack, InitializerList_Constructor) {
  Stack<unsigned int> s1{1, 2, 3, 4};
  ASSERT_EQ(s1.size(), 4u);
  ASSERT_EQ(s1.top(), 4u);
  Stack<string> s2{"morning", "village", "people"};
  ASSERT_EQ(s2.size(), 3u);
  ASSERT_EQ(s2.top(), "people");
}

TEST(Suite_Stack, InitializerList_Constructor_Pair) {
  Stack<pair<string, int>> s21_s1{{"one", 1}, {"two", 2}, {"three", 3}};
  stack<pair<string, int>> std_s2;
  std_s2.push({"one", 1});
  std_s2.push({"two", 2});
  std_s2.push({"three", 3});
...
TEST(Suite_Stack, Empty_Pair) {
  Stack<pair<string, int>> s21_s;
  stack<pair<string, int>> std_s;
...
TEST(Suite_Stack, Size) {
  Stack<pair<string, unsigned int>> s21_s;
  stack<pair<string, unsigned int>> std_s;
...
TEST(Suite_Stack, CopyConstructor) {
  ...
  stack<int> std_copy(std_stack);
  Stack<int> s21_copy(s21_stack);
...
TEST(Suite_Stack, Copy_Constructor_String) {
  Stack<string> s21_stack_1;
  stack<string> std_stack_1;
  s21_stack_1.push("one");
  std_stack_1.push("one");
  s21_stack_1.push("two");
  std_stack_1.push("two");
  Stack<string> s21_stack_2(s21_stack_1);
  stack<string> std_stack_2(std_stack_1);
...
TEST(Suite_Stack, Copy_Constructor_Pair) {
  ...
  Stack<pair<string, int>> s2(s1);
  stack<pair<string, int>> s2_stl(s1_stl);
...
TEST(Suite_Stack, Move_Constructor_Pair) {
  ...
  s1.push({"two", 2});
  s1_stl.push({"two", 2});
  Stack<pair<string, int>> s2(move(s1));
  stack<pair<string, int>> s2_stl(move(s1_stl));
...
TEST(Suite_Stack, Move_Constructor_String) {
  ...
  Stack<string> s2(move(s1));
  stack<string> s2_stl(move(s1_stl));
...
TEST(Suite_Stack, Copy_Assignment_Operator) {
  ...
  Stack<int> my_copy_stack;
  stack<int> stl_copy_stack;
...
TEST(Suite_Stack, CopyAssignmentOperatorPair) {
  ...
  Stack<pair<string, int>> s2;
  stack<pair<string, int>> s2_stl;
...
TEST(Suite_Stack, Move_Assignment_Operator) {
  ...
  Stack<int> my_move_stack;
  stack<int> stl_move_stack;
  my_move_stack = move(my_stack);
  stl_move_stack = move(stl_stack);
